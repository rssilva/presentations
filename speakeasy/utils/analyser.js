...
class Analyser {
  ...
  drawTime () {
    this.timeId = window.requestAnimationFrame(() => this.drawTime())
  ...
  drawFrequency () {
    this.freqId = window.requestAnimationFrame(() => this.drawFrequency())
  ...
  clear() {
    ...
    if (this.freqId) {
      window.cancelAnimationFrame(this.freqId)
    }
